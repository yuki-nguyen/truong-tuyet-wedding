...
function scheduleWish(delay){clearTimeout(wishTimer);wishTimer=setTimeout(function(){
 if(wishQueue.length&&!wishHidden&&getComputedStyle(stream).display!=='none'&&!document.body.classList.contains('rsvp-open')){
  ...
  // Match the reference: the next card follows about 10px behind this one.
  lastWishSpacing=Math.max(1400,((height||90)+10)/28*1000);
  scheduleWish(lastWishSpacing);
...
(function(){
  ...
  open.onclick=openOverlay;var dockOpen=document.getElementById('rsvpDock');if(dockOpen)dockOpen.onclick=openOverlay;close.onclick=closeOverlay;back.onclick=closeOverlay;document.addEventListener('keydown',function(e){if(e.key==='Escape'&&ov.classList.contains('open'))closeOverlay()});
