(function(){'use strict';var $=function(s){return document.querySelector(s)},$$=function(s){return Array.prototype.slice.call(document.querySelectorAll(s))};var envelope=$('#envelope'),dock=$('#dock'),music=$('#musicBtn'),stream=$('#wishStream'),backdrop=$('#backdrop'),wishModal=$('#wishModal'),giftModal=$('#giftModal');var player=null,playerReady=false,musicOn=false;
...
function tick(){var raw=new Date('2026-11-07T00:00:00+07:00').getTime()-Date.now(),d=Math.max(0,raw),vals=[Math.floor(d/86400000),Math.floor(d%86400000/3600000),Math.floor(d%3600000/60000),Math.floor(d%60000/1000)];$$('#count b').forEach(function(x,i){x.textContent=String(vals[i]).padStart(2,'0')});var today=$('#countdownToday');if(today)today.hidden=raw>0}tick();setInterval(tick,1000);
